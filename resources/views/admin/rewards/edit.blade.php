    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.rewards.update", [$reward->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee_name') ? 'is-invalid' : '' }}" name="employee_name_id" id="employee_name_id" required>
                    @foreach($employee_names as $id => $employee_name)
                        <option value="{{ $id }}" {{ ($reward->employee_name ? $reward->employee_name->id : old('employee_name_id')) == $id ? 'selected' : '' }}>{{ $employee_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reward') ? 'is-invalid' : '' }}" type="text" name="reward" id="reward" value="{{ old('reward', $reward->reward) }}" required>
            ...
            <label class="required" for="description">Description</label>
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $reward->description) }}</textarea>
