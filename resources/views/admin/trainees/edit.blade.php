...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trainee.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trainees.update", [$trainee->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee_name') ? 'is-invalid' : '' }}" name="employee_name_id" id="employee_name_id" required>
                    @foreach($employee_names as $id => $employee_name)
                        <option value="{{ $id }}" {{ ($trainee->employee_name ? $trainee->employee_name->id : old('employee_name_id')) == $id ? 'selected' : '' }}>{{ $employee_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('programme_name') ? 'is-invalid' : '' }}" type="text" name="programme_name" id="programme_name" value="{{ old('programme_name', $trainee->programme_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue') ? 'is-invalid' : '' }}" type="text" name="venue" id="venue" value="{{ old('venue', $trainee->venue) }}" required>
                @if($errors->has('venue'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('time_date') ? 'is-invalid' : '' }}" type="text" name="time_date" id="time_date" value="{{ old('time_date', $trainee->time_date) }}" required>
