...
<head>
    ...
    <link rel="icon" href="/uploads/logo/logo.jpg" type="/uploads/logo/x-icon">
...
<body class="sidebar-mini layout-fixed" style="height: auto;">
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
            ...
            <a href="#" >
               <span class="font-weight-strong">MUST-STAFF PERFORMANCE  APPRAISAL SYSTEM</span>
           </a>
           @if(Auth::check())    
            ...
            <div class="dropdown" style=" margin-left:400px;">
                <a href="#" class="dropdown-toggle " data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><i class="fas fa-bell"></i>
                    Notifications <span class="badge badge-danger" id="count-notification">{{ auth()->user()->unreadNotifications->count() }}</span> <span class="caret"></span>
                </a> 

                <ul class="dropdown-menu" role="menu" aria-labelldby="navbarDropdown">
                  @if(auth()->user()->unreadNotifications->count())
  
                            @foreach(auth()->user()->unreadNotifications as $notification)
                              <li >
                              @if(isset($notification->data['reward']['reward']))
                                  <a href="{{ route('admin.rewards.show', [$notification->data['reward']['id']]) }}" >
                                  {{ $notification->data['reward']['reward'] }} 
                                  </a>
                              </li>
                            @elseif(isset($notification->data['training']['programme_name']))
                            <li>
                                <a href="{{ route('admin.trainees.show', [$notification->data['training']['id']]) }}" >
                                {{ $notification->data['training']['programme_name'] }} 
                                </a>
                            </li>
                            @elseif(isset($notification->data['performance']['emp_rating1']))
                            <li>
                                <a href="{{ route('admin.performances.show',  [$notification->data['performance']['id']]) }}" >
                                <p hidden>{{ $notification->data['performance']['user_id'] }}</p> New appraisal
                                </a>
                            </li>
                            @elseif(isset($notification->data['notifysenior']['percentage'] ))
                            <li>
                                <a href="{{ route('admin.performances.show',  [$notification->data['notifysenior']['id']]) }}" >
                                You scored {{ $notification->data['notifysenior']['percentage'] }} %
                                </a>
                            </li>
                            @elseif(isset($notification->data['support']['emp_rating1'])) 
                            <li>
                                <a href="{{ route('admin.support-staff-appraisals.show', [$notification->data['support']['id']]) }}" >
                                <p hidden>{{ $notification->data['support']['user_id'] }}</p> New appraisal
                                </a>
                            </li>
                            @elseif(isset($notification->data['notifysupport']['percentage'] ))
                                    <a href="{{ route('admin.support-staff-appraisals.show', [$notification->data['notifysupport']['id']]) }}" >
                                    You scored {{ $notification->data['notifysupport']['percentage'] }} %
                                    </a>
                            @endif
                            @endforeach 
      ...
      @endif
        ...
        <footer class="main-footer">
            ...
            <strong> MUST-SPA &copy;</strong>{{ trans('global.allRightsReserved') }}
        </footer>
